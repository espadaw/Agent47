...
export class VirtualsConnector extends BasePlatformConnector {
    async fetchJobs(filters) {
        try {
            ...
            const { default: AcpClient, AcpContractClient, baseAcpConfig, baseSepoliaAcpConfig, AcpGraduationStatus } = await import('@virtuals-protocol/acp-node');
            ...
            const contractClient = new AcpContractClient(entityId, baseAcpConfig);
            // Initialize Contract Client
            // defaulting sessionKeyId to 0, which might fail if not properly set up
            await contractClient.init(formattedPrivateKey, 0);
            const client = new AcpClient({
                acpContractClient: contractClient,
                skipSocketConnection: true
            });
            // Browse available agents on the network
            const agents = await client.browseAgents(filters?.query || '', { graduationStatus: AcpGraduationStatus.GRADUATED });
