...
export class AgentAIScraper extends BaseScraper {
    ...
    categorizeAgent(skills) {
        if (!skills || skills.length === 0)
            return JobCategory.OTHER;
        const skillsStr = skills.join(' ').toLowerCase();
        if (skillsStr.includes('code') || skillsStr.includes('develop'))
            return JobCategory.DEVELOPMENT;
        if (skillsStr.includes('data') || skillsStr.includes('analysis'))
            return JobCategory.DATA;
        if (skillsStr.includes('design') || skillsStr.includes('creative'))
            return JobCategory.CREATIVE;
        if (skillsStr.includes('write') || skillsStr.includes('content'))
