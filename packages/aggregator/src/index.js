import { RentAHumanConnector, X402Connector, JobForAgentScraper, VirtualsConnector } from '@agent47/integrations';
...
export class AggregatorEngine {
    ...
    async fetchAllJobs(filters) {
        ...
        const promises = this.connectors.map(connector => connector.fetchJobs(filters).catch((err) => {
