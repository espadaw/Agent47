...
import { z } from "zod";
import { AggregatorEngine } from './aggregator/index.js'; // Note .js extension for tsx/esm
...
server.tool(
    ...
    async ({ query, minPrice, maxPrice, platform }) => {
        console.error(`[MCP] Handling findJobs request:`, { query, minPrice, maxPrice, platform }); // Log to stderr to not mess up stdout JSON-RPC
